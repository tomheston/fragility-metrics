...
| **SFM**            | Scaling    | >0, ≠1 | Secondary            | Factor k to flip (k > 1 flips nonsignificant results; k < 1 flips significant results) | Sample size fragility multiplier                  |
...
**Definition**: NBF-based robustness metric measuring distributional distinguishability for continuous outcomes, based on the crossover point of the two sample distributions. MeCI is p-value independent and sample-size independent.

**Formula**: Let μ₁, μ₂ be the observed group means and s₁, s₂ their standard deviations.   

* Calculate the distributional crossover point:   - c = (s₁·μ₂ + s₂·μ₁) / (s₁ + s₂)   Calculate the raw distance:   - d = min(|μ₁ − c|, |μ₂ − c|) / √(s₁² + s₂²)   
...
**Neutrality**: μ₁ = μ₂ (the nearer group mean sits at the crossover point; d = 0, MeCI = 0).   
...
**Note**: Gold standard for binary and multinomial tables: path-independent, label-invariant, and domain-complete (defined for every table except the small-N floor, where no arrangement of N subjects can reach significance at the chosen α). Computed exactly by vectorized shell search: all candidate tables at reallocation distance d are evaluated in closed form (chi-square) or via cached exact Fisher p-values, for d = 1, 2, 3, … until the classification flips. This resolves tables at trial scale in well under a second (e.g., GFI ≈ 80 at N = 10,000), so no fallback metric is required.
...
- **Index forms** (raw counts/distances): GFI, FI, MFI, SFI, UFI, PFI
